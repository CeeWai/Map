...
import os, time, requests, geocoder, json, datetime, pickle, os.path, secrets
...
def index():
    ...
    return render_template('map.html', eventList=eventList, event_locations=event_locations, event_markers_image=event_markers_image, pointList=pointList, latlng=latlng)
...
def editEvent(eventid):
    ...
    if form.validate_on_submit():
        ...
        new_event = Event(title=form.title.data, area=form.area.data, start_date=form.start_date.data,
                          end_date=form.end_date.data, desc=form.desc.data, owner_id=current_user.id, event_image=picture_file)
        db.session.add(new_event)
        db.session.commit()
